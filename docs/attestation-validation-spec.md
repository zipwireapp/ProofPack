...
∗ **Implementation note:** The **.NET implementation** checks “schema recognized” in Stage 1 shared code. Checks for **not expired** and **not revoked** are delegated to specialists because expiry/revocation metadata reside only on-chain (in the fetched EAS attestation), not in the ProofPack payload. A shared Stage 1 check would require an extra on-chain fetch in the pipeline before specialist dispatch, which is inefficient. Instead, each specialist (IsDelegate, EAS Private Data, etc.) enforces these checks when it has access to the on-chain data. This is a pragmatic deviation from the theoretical spec (which treats all three as “Stage 1”) but is justified by implementation constraints: the payload document cannot carry full on-chain state. Validation completeness is preserved: expiry and revocation are still enforced, by specialists, for every attestation. For how EAS timestamp sentinels (e.g. uint64(0) → UnixEpoch) are handled in .NET, see [Unix timestamp sentinels](unix-timestamp-sentinels.md).

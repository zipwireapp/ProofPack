...
The first is that cryptography is a niche within software engineering, so dealing with cryptographic proofs doesn’t come naturally to the coders making our digital products who are focused on their area of product and design specialism.


The second is that we need to converge on a single way to represent this data; we need a data format everyone understands, like how we can all speak PDF.


The third is that we need awareness within product development teams that selective-disclosure is possible, easy, and should be the default right thing to do. Apps and websites need to accept a proof and the apps that are fact sources need to produce proofs. Proofs are far more robust than someone merely eyeballing bank_statement_1.pdf, ‘[shrug], hmm looks legit’.
...
https://www.npmjs.com/package/@zipwire/proofpack
https://www.npmjs.com/package/@zipwire/proofpack-ethereum

And for .NET on NuGet.org
https://www.nuget.org/packages/Zipwire.ProofPack
https://www.nuget.org/packages/Zipwire.ProofPack.Ethereum
...
Spread the word. ProofPack. Let’s stop this oversharing madness.
